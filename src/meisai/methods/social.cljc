(ns meisai.methods.social
  "social.cljc — 明細 — Member Card-Statement Ingestion DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from meisai's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "明細 — Member Card-Statement Ingestion が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over meisai's own domain (Member-own card-statement (利用明細) ingestion into the local kotoba Datom log. The member runs a READ-ONLY computer-use fetch leg on their OWN…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "meisai R0: live social posting is Council Lv6+ + operator + "
